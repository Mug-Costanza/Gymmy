...
class WorkoutForm(forms.ModelForm):
    ...
    if day and month and year:
        try:
            # Convert day, month, and year to integers and create a date object
            day = int(day)
            month = int(month)
            year = int(year)
            return datetime.date(year, month, day)
        except ValueError:
            # This will catch cases like February 30
            raise forms.ValidationError('Invalid date.')
    else:
        # If any of the day, month, or year is missing
        raise forms.ValidationError('Complete the date fields.')

    def clean(self):
        cleaned_data = super().clean()
        try:
            self.clean_date()  # Call clean_date method to validate the date field
        except forms.ValidationError as e:
            self.add_error('date', e)  # Add error to 'date' field if validation fails
        return cleaned_data
